...
export default function RegionCom() {
  ...
  const {
    ...
    [reactQueryKey.getRegion, region.search],
    getRegion,
    region.search
  );
  ...
  const onPageChange = (page: number, pageSize: number) => {
    dispatch(
      changeSetting({
        region: { ...region, search: { page, limit: pageSize } },
      })
    );
  };
  ...
  const onSelectRow = (record: Region) => {
    if (region.selectedRowKeys[0] === record.id) {
      ...
      dispatch(
        changeSetting({
          region: { ...region, form: obj, selectedRowKeys: [record.id] },
        })
      );
    }
  };
    ...
    <div className="set-region">
      <div className="set-region-left">
        ...
        <Table
          ...
          rowKey="id"
          rowClassName={(record) =>
            record.id === region.selectedRowKeys[0] ? "active-row" : ""
          }
          dataSource={regionInfo?.pages[regionInfo.pages.length - 1].items}
          columns={columns}
      ...
      <div className="set-region-right">
          ...
          onValuesChange={(changeValues) =>
            dispatch(
              changeSetting({
                region: {
                  ...region,
                  form: { ...region.form, ...changeValues },
                },
              })
            )
          }
          initialValues={region.form}
          ref={formListRef}
          formList={formList}
          col={{ span: 24 }}
          labelSpan={7}
        ...
        <div className="set-region-right-btn">
          <ToolBtn
            onClick={() => onOptRegion("add")}
            src={commenBtn}
            acSrc={commenAcBtn}
            content="增加"
          />
          <ToolBtn
            onClick={() => onOptRegion("edit")}
            src={commenBtn}
            acSrc={commenAcBtn}
            content="修改"
          />
